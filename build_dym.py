...
#
# 运行：
#   python app.py --data ./matches --champion-json ./champion.json --port 5000
#
# 页面：
#   /players                 分路-选手（可点选手进入对局明细）
#   /player?name=xxx         选手对局明细
#   /champions               分路-英雄（可点英雄进入对局明细）
#   /champion/<champion_id>  英雄对局明细（含被Ban对局列表）
#   /teams                   队伍排行榜
#   /team?name=xxx           队伍参与的所有比赛 + 队伍选用英雄排行 + 队伍Ban英雄排行
#   /bans                    Ban 榜
#   /match/<match_id>        单局全员详情
...
import pandas as pd
from flask import Flask, render_template_string, url_for, request, abort, redirect
        ...
        if isinstance(obj, list):
            for it in obj:
                if isinstance(it, dict):
                    ...
                    raise RuntimeError(f"列表元素不是对象: {fp}, type={type(it)}")
        elif isinstance(obj, dict):
            obj["_src_file"] = fp
            matches.append(obj)
        else:
            raise RuntimeError(f"不支持的JSON结构: {fp}, type={type(obj)}")
    return matches
...
def load_champion_map_from_champion_json(champion_json_path: str) -> Dict[str, str]:
    ...
    if not isinstance(data, dict):
        raise RuntimeError("champion.json 格式不正确：缺少 data 字段或 data 不是对象")
    ...
    if not mp:
        raise RuntimeError("从 champion.json 未解析到任何英雄映射（key/name）")
    return mp
...
<head>
  ...
  <style>
    ...
    .nav-pills.lane-pills{
      gap:.65rem;
      flex-wrap:wrap;
    }

    .nav-pills.lane-pills .nav-link{
      border:none;
      border-radius:999px;
      padding:.65rem 1rem;
      font-weight:600;
    ...
    .nav-pills.lane-pills .nav-link.active{
    ...
    .nav-pills.lane-pills .nav-link:not(.active) .lane-badge{
      background:#e2e8f0;
      color:#334155;
    }
    ...
    .badge-win{ color:#166534; background:#dcfce7; }
    .badge-lose{ color:#991b1b; background:#fee2e2; }
...
<nav class="navbar navbar-expand-lg navbar-dark">
  <div class="container-fluid px-4">
    <a class="navbar-brand" href="{{ url_for('players') }}">HVV杯 S12 比赛统计</a>
    <button class="navbar-toggler" type="button" data-bs-toggle="collapse" data-bs-target="#topNav">
      <span class="navbar-toggler-icon"></span>
    </button>
    <div class="collapse navbar-collapse" id="topNav">
      <div class="navbar-nav ms-auto">
        <a class="nav-link {% if active=='players' %}active{% endif %}" href="{{ url_for('players') }}">选手数据</a>
        <a class="nav-link {% if active=='champions' %}active{% endif %}" href="{{ url_for('champions') }}">英雄数据</a>
        <a class="nav-link {% if active=='teams' %}active{% endif %}" href="{{ url_for('teams') }}">队伍榜</a>
        <a class="nav-link {% if active=='bans' %}active{% endif %}" href="{{ url_for('bans') }}">Ban榜</a>
      </div>
    </div>
  </div>
</nav>
    ...
    <div class="card-body p-4 p-lg-5">
      <div class="row g-4 align-items-center">
        <div class="col-lg-8">
          ...
          {% endif %}
          <div class="meta">
            英雄映射：<code>{{ champion_json_path }}</code>
          </div>
        </div>
        <div class="col-lg-4 text-lg-end">
          <a class="btn btn-light btn-sm me-2 mb-2" href="{{ url_for('players') }}">选手榜</a>
          <a class="btn btn-light btn-sm me-2 mb-2" href="{{ url_for('champions') }}">英雄榜</a>
          <a class="btn btn-light btn-sm me-2 mb-2" href="{{ url_for('teams') }}">队伍榜</a>
          <a class="btn btn-light btn-sm mb-2" href="{{ url_for('bans') }}">Ban榜</a>
        </div>
      </div>
    </div>
  </div>
...
def create_app(data_path: str, champion_json_path: str) -> Flask:
    app = Flask(__name__)

    champ_map = load_champion_map_from_champion_json(champion_json_path)
    matches = load_matches(data_path)
    total_matches = len(matches)

    match_meta = build_match_meta(matches)

    df_detail = extract_rows(matches, champ_map=champ_map, match_meta=match_meta)
    players_df = agg_players(df_detail)
    champs_df = agg_champions(df_detail)
    team_match_stats = build_team_match_stats(df_detail)
    teams_df = agg_teams(team_match_stats)

    ban_df = extract_bans(matches)
    if not ban_df.empty:
        ban_df = ban_df.copy()
        ban_df["team_name"] = ban_df.apply(
            lambda r: team_name_from_meta(match_meta, r["match_id"], r["teamId"]),
            axis=1
        )
        ban_df["champion_name"] = ban_df["champion_id"].map(lambda x: champ_map.get(str(x), str(x)))

    ban_stats = make_ban_stats(ban_df, total_matches=total_matches)
    if not ban_stats.empty:
        ban_stats["champion_name"] = ban_stats["champion_id"].map(lambda x: champ_map.get(str(x), str(x)))

    team_pick_champs_df = agg_team_pick_champions(df_detail)
    team_ban_champs_df = agg_team_ban_champions(ban_df)

    if not team_match_stats.empty:
        df_team_matches = team_match_stats.copy()
    else:
        df_team_matches = pd.DataFrame(columns=[
            "match_id", "match_ord", "match_date", "match_no",
            "teamId", "team_name", "opponent",
            "win", "avg_score", "team_kills", "team_deaths", "team_assists",
            "team_kda", "avg_kp", "team_gold", "avg_level",
    ...
    def team_link(team_name: str) -> str:
        if team_name is None or (isinstance(team_name, float) and pd.isna(team_name)):
            return "-"
        return f'<a href="{url_for("team_detail", name=str(team_name))}">{html_escape(str(team_name))}</a>'

    def player_link(player_name: str) -> str:
        return f'<a href="{url_for("player_detail", name=str(player_name))}">{html_escape(str(player_name))}</a>'

    def champion_link(champion_id: str, champion_name: str) -> str:
        return f'<a href="{url_for("champion_detail", champion_id=str(champion_id))}">{html_escape(str(champion_name))}</a>'

    def match_link(match_id: str) -> str:
        return f'<a href="{url_for("match_detail", match_id=str(match_id))}">{html_escape(str(match_id))}</a>'
    ...
    def players():
        ...
        summary_cards = [
            {"label": "总对局数", "value": total_matches},
            {"label": "参赛记录", "value": len(df_detail) if not df_detail.empty else 0},
            {"label": "选手数", "value": df_detail["player"].nunique() if not df_detail.empty else 0},
            {"label": "队伍数", "value": df_detail["team_name"].nunique() if not df_detail.empty else 0},
        ]
    ...
    def player_detail():
        ...
        if not name:
            abort(400, "缺少 name 参数：/player?name=选手昵称")

        dff = df_detail[df_detail["player"] == name].copy()
        if dff.empty:
            abort(404, f"找不到选手：{name}")
        ...
        dff = dff.sort_values(["match_ord"], ascending=[False])

        table_html = df_to_table(
            dff,
            columns=[
                "match_link", "match_date", "match_no",
                "lane", "team_link", "result",
                "champ_link", "opp_player", "opp_champion",
        ...
        content = build_section_card(
            ...
            actions_html=f'<a class="btn btn-soft" href="{url_for("players")}">返回选手榜</a>',
    ...
    def champions():
        ...
        summary_cards = [
            {"label": "总对局数", "value": total_matches},
            {"label": "已上场英雄", "value": df_detail["champion_name"].nunique() if not df_detail.empty else 0},
            {"label": "有Ban记录英雄", "value": ban_stats["champion_id"].nunique() if not ban_stats.empty else 0},
            {"label": "总Ban次数", "value": len(ban_df) if not ban_df.empty else 0},
        ]
    ...
    def champion_detail(champion_id: str):
        cid = str(champion_id).strip()
        champ_name = champ_map.get(cid, cid)

        played = df_detail[df_detail["champion_id"].astype(str) == cid].copy()
        if not played.empty:
            ...
            played = played.sort_values(["match_ord"], ascending=[False])

            played_table = df_to_table(
                played,
                columns=[
                    "match_link", "match_date", "match_no",
                    "player_link", "lane", "team_link", "result",
                    "opp_player", "opp_champion",
                    "score", "kda_str", "kpr", "gold", "level",
                    "gold_diff", "level_diff",
                ],
                col_rename={
        ...
        banned = pd.DataFrame()
        if not ban_df.empty:
            banned = ban_df[ban_df["champion_id"].astype(str) == cid].copy()

        if not banned.empty:
            banned = banned.drop_duplicates(subset=["match_id", "teamId"], keep="first")
            banned["match_link"] = banned["match_id"].map(match_link)
            banned["ban_team"] = banned.apply(lambda r: team_name_from_meta(match_meta, r["match_id"], r["teamId"]), axis=1)
            banned["ban_team_link"] = banned["ban_team"].map(team_link)
            banned["match_ord"] = banned["match_id"].map(lambda mid: match_ord_map.get(mid, 0))
            ...
            banned_html = '<div class="empty-state">该英雄暂无被 Ban 记录（或数据中没有 Ban 信息）。</div>'

        banrate = "-"
        ban_matches = 0
        bans = 0
        if not ban_stats.empty:
            row = ban_stats[ban_stats["champion_id"].astype(str) == cid]
            if not row.empty:
                r0 = row.iloc[0]
        ...
        content = (
            build_section_card(
                ...
                actions_html=f'<a class="btn btn-soft" href="{url_for("champions")}">返回英雄榜</a>',
    ...
    def teams():
        ...
        summary_cards = [
            ...
            {"label": "队伍参赛记录", "value": len(team_match_stats) if not team_match_stats.empty else 0},
        ...
        if dff.empty:
            ...
            content = build_section_card(
                title="队伍排行榜",
                subtitle="点击队伍可查看该队伍参与的全部比赛与单场队伍数据。",
                body_html=f'<div class="table-wrap">{table_html}</div>'
            )
    ...
    def team_detail():
        name = request.args.get("name", "").strip()
        if not name:
            abort(400, "缺少 name 参数：/team?name=队伍名")

        dff = df_team_matches[df_team_matches["team_name"] == name].copy()
        if dff.empty:
            abort(404, f"找不到队伍或队伍暂无对局：{name}")

        team_row = teams_df[teams_df["team_name"] == name].copy()
        ...
        pick_df = team_pick_champs_df[team_pick_champs_df["team_name"] == name].copy()
        ...
        team_ban_df = team_ban_champs_df[team_ban_champs_df["team_name"] == name].copy()
        ...
        content = (
            build_section_card(
                ...
                actions_html=f'<a class="btn btn-soft" href="{url_for("teams")}">返回队伍榜</a>',
                body_html=f'<div class="table-wrap">{table_html}</div>'
            )
            +
            build_section_card(
    ...
    def match_detail(match_id: str):
        dff = df_detail[df_detail["match_id"].astype(str) == str(match_id)].copy()
        if dff.empty:
            abort(404, f"找不到对局：{match_id}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="本地 Data Dragon champion.json 路径（默认当前目录 champion.json）",
    )
    args = parser.parse_args()

    app = create_app(args.data, args.champion_json)
